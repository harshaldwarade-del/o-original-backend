  ...
  try {
    ...
    if (!devID || !password) {
      return res.status(400).json({
        success: false,
        message: "Please provide devID and password.",
      });
    }
    ...
    if (!user || !(await user.comparePassword(password))) {
      return res
        .status(401)
        .json({ success: false, message: "Invalid devID or password." });
    }
...
exports.register = async (req, res) => {
  try {
    ...
    const existing = await User.findOne({
      $or: [{ devID: devID?.toUpperCase() }, { email }],
    });
    if (existing) {
      return res
        .status(409)
        .json({ success: false, message: "devID or email already exists." });
    }
